...
public class REDCHINS_FOURTRAP extends Script{
    ...
    final int boxTrapSuccessID = 9383;    // On ground success (green)
    final int boxtrapIgnoreID = 9390; //it goes to this ID for a split second before success, so ignore it
    final int boxtrapIgnoreID2= 9381; //it goes to this ID for a split second before fail, so ignore it
    final int boxtrapIgnoreID3= 9382; //it goes to this ID for a split second before fail, so ignore it


    int maxNumberOfTraps = 4;
    ...
    static ArrayList<Location> locations = new ArrayList<Location>() {
        {
            add(new Location(2506, 2885, 0));
            add(new Location(2508, 2885, 0));
            add(new Location(2507, 2886, 0));
            add(new Location(2507, 2884, 0));
            ...
            switch (taskToHandle) {
                ...
                case PlaceTrap:
                    ...
                    if (Player.getPosition().getX() != openLocation.tile.getX() || Player.getPosition().getY() != openLocation.tile.getY()) {
                        ...
                        //General.sleep(General.randomSD(500, 600, 500, 1100));
                    }
                    ...
                    if (boxTrapInInventory.length > 0) {
                        ...
                        //General.sleep(General.randomSD(500,1000, 750, 750));
                    }
...
//                    this.abc_util.generateTrackers();
                    ...
                    Timing.waitCondition(()-> {General.sleep(200, 300); return isThereSomethingToDo();}, General.randomSD(60000, 65000, 62000, 62000));
                ...
                case ResetTrap:
                    ...
                    General.sleep(General.randomSD(800,1000, 800, 800));
//                    while (Player.getAnimation() == -1) {
//                        General.sleep(20,50);
//                        System.out.println("animation is -1 because reset pause");
//                    }
                    ...
                    System.out.println("FINISHED RESETTING TRAP");
                ...
                case LayTrap:
                    ...
                    System.out.println("FINISHED LAYING TRAP");
    ...
    public Trap.STATE determineStateOfTrap(Trap trap){
        ...
        for(RSObject object : objectsOnTrap){
            if(object.getID() == boxTrapWaitID)
                ...
                return Trap.STATE.Ground;
            else if(object.getID() == boxtrapIgnoreID) {
                System.out.println("TRAP IGNORE");
                return Trap.STATE.Wait;}
            else if(object.getID() == boxtrapIgnoreID2) {
                System.out.println("TRAP IGNORE");
                return Trap.STATE.Wait;}
        }
    ...
    public boolean isThereSomethingToDo () {
        for (Iterator<Trap> trapsItr = trapsPlacedArray.iterator(); trapsItr.hasNext();) {
            ...
            if (trap.currentState != Trap.STATE.Wait) {
                ...
                if(trap.currentState == Trap.STATE.NA){
