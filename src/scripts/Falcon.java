...
public class Falcon extends Script implements MessageListening07 {
    ...
    //replace hardcoded spotted kebbits ids with this when making the script abstract to various kebbit
    final int kebbitIDToFind = 0;
    ...
    public void retrieveFalcon() {
        RSNPC[] theFalcon = NPCs.find(falcon);
    ...
    public RSNPC findSpottedKebbit () {
        RSNPC[] nearbySpottedKebbits = NPCs.findNearest(spottedKebbit);
    ...
    public void dropSpottedLoot() {
        shiftDrop.shiftDrop(bones, spottedKebbitFur);
    }

    //method to find falcon on the floor after it successfully catches kebbit
    public RSNPC findFalcon() {
        final RSNPC[] theFalcon = NPCs.find(falcon);

        System.out.println("found the falcon");
        return theFalcon[0];
    }
}
